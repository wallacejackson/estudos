...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> createOrUpdate(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			...
			Ticket ticketPersisted = service.createOrUpdate(ticket);
	...
	public ResponseEntity<Response<Ticket>> update(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			...
			Ticket ticketCurrent = service.findById(ticket.getId());
			...
			Ticket ticketPersisted = (Ticket) service.createOrUpdate(ticket);
			response.setData(ticketPersisted);
		} catch (Exception e) {
			response.getErros().add(e.getMessage());
			return ResponseEntity.badRequest().body(response);
	...
	public ResponseEntity<Response<Ticket>> findById(@PathVariable("id") String id) {
		...
		try {
			...
			if (ticket == null) {
				response.getErros().add("Register not fount id: " + id);
				return ResponseEntity.badRequest().body(response);
			}
			List<ChangeStatus> changes = new ArrayList<ChangeStatus>();
			Iterable<ChangeStatus> changesCurrent = service.listChangeStatus(ticket.getId());
	...
	public ResponseEntity<Response<Page<Ticket>>> findAll(HttpServletRequest request, @PathVariable("page") int page,
		...
		if (userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
			tickets = service.listTicket(page, count);
		} else if (userRequest.getProfile().equals(ProfileEnum.ROLE_CUSTOMER)) {
			tickets = service.findByCurrentUser(page, count, userRequest.getId());
		}
	...
	public ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request,
		...
		if (number > 0) {
			tickets = service.findByNumber(page, count, number);
		} else {
			User userRequest = userFromRequest(request);
			if (userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				if (assigned) {
					tickets = service.findByParameterAndAssignedUser(page, count, title, status, priority,
							userRequest.getId());
				} else {
					tickets = service.findByParameters(page, count, title, status, priority);
				}
			} else if (userRequest.getProfile().equals(ProfileEnum.ROLE_CUSTOMER)) {
				tickets = service.findByParameterAndAssignedUser(page, count, title, status, priority,
						userRequest.getId());
	...
	public ResponseEntity<Response<Summary>> findSummary() {
		...
		Iterable<Ticket> tickets = service.findAll();
		if (tickets != null) {
